...
```bash
cd /workspace/hunter
python3 scripts/compare_runs.py --log-dir runs --prefix standing_pd_ext --limit 10 --sort-by survival
```

* `--sort-by mtime`（新しい順）
* `--sort-by survival`（生存時間が長い順）
* `--sort-by tilt`（傾き最大が小さい順）
* `--sort-by energy`（消費が小さい順）
