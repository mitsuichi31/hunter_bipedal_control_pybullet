...
class ObserverConfig:
    ...
    contact_params: ContactEstimatorParams = field(default_factory=ContactEstimatorParams)
...
class Observer:
    ...
    def __init__(self, config: ObserverConfig | None = None):
        ...
        self.filters = StateFilter(
            base_alpha=self.config.base_alpha,
            joint_alpha=self.config.joint_alpha,
        )
        self.contact_estimator = ContactEstimator(self.config.contact_params)
    ...
    def update(
        ...
        ordered = [contact_forces.get("left", np.zeros(3)), contact_forces.get("right", np.zeros(3))]
